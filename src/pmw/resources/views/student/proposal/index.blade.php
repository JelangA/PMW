                        ...
                        <table id="crudProposal" class="table table-striped w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">{{ __('Skema') }}</th>
                                    <th class="text-center">{{ __('Bidang Bisnis') }}</th>
                                    <th class="text-center">{{ __('Nama Tim') }}</th>
...
@push('scripts')
    <script>
        var datatable = $('#crudProposal').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('complete-proposal.index') }}",
            columns: [
                { data: 'no', name: 'no', render: function (data, type, row, meta) {
                        return meta.row + meta.settings._iDisplayStart + 1;
                }, width: '5%', class: 'text-center' },
                { data: 'scheme_name', name: 'scheme_name', class: 'text-center', width: '15%' } ,
                { data: 'schemes_type_name', name: 'schemes_type_name', class: 'text-center' },
                { data: 'team_name', name: 'team_name', class: 'text-center' },
